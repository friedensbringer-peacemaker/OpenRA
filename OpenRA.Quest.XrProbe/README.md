...
Für den späteren Gerätetest installiert `scripts/smoke-quest-xr.sh install ../Artifacts/OpenRA-Quest-XR-Combined-untested.apk` die kombinierte APK und startet sie. Nach dem manuellen Import eigener Red-Alert-Daten und dem Start der XR-Fläche sichert `scripts/smoke-quest-xr.sh capture` Logcat, Activity-Zustand, System-Screenshot und vorhandene OpenRA-Diagnosebilder. Das Skript wurde noch nicht an einem verbundenen Gerät ausgeführt. Die sichtbare Quad-Fläche muss zusätzlich im Headset beurteilt werden; ein Android-System-Screenshot muss sie nicht zwingend enthalten.
