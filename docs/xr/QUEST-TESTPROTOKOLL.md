...
| --- | --- | --- | --- |
| 1. App/Daten | App startet; Red-Alert-ZIP nötigenfalls über „Red-Alert-ZIP auswählen“ importieren. Danach erscheint „Spiel wird geladen …“ mit Sekundenanzeige und bei Erfolg „Red-Alert-Partie läuft“ mit Ladezeit. | `xr-logcat.txt`: Regeln/Karte oder konkreter Importfehler; Screenshot der Android-Ansicht. | **Pass** nur bei erfolgreichem Import bzw. nachweislich vorhandenen Daten; sonst **Fail** mit Fehlermeldung. |
...
Bei einem **Fail** die letzte funktionierende Stufe benennen und den vollständigen Ausgabeordner behalten. Zusätzlich `OpenRA.Quest.Probe/scripts/collect-quest-logs.sh` ausführen: Es exportiert `quest-diagnostics.log` aus dem privaten App-Speicher, relevante Android-Logs und den Crash-Puffer in einen lokalen Ordner. Die Diagnose-Datei nennt die letzte erreichte Startphase und verwaltete Ausnahmen; native Abstürze und ANRs müssen im Android-Log geprüft werden. `activity-state.txt` hilft bei Activity-/Fokusfragen; die zwei OpenRA-Diagnose-PNGs zeigen nur die gespeicherten Android-Frames. Im Log nach `AndroidRuntime`, `OpenRA.Quest.Probe`, `OpenRA.XrProbe` und `XR-Bildübergabe` suchen.
